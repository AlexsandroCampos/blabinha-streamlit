...
def create_chat(chat):
    ...
    st.title("" +  chat['name'] + " Blabinha Versão :blue[Azul]")
    st.header("Testador: :blue[" + st.session_state["nome"]+ "]")

    # Mudar isso: (precisa aparecer nos novos e chats e não pode mudar durante a conversa)
 
    strategies = ["Zero-Shot", "One-Shot", "Few-Shots", "Step-By-Step", "Chain-of-Thought", "Self-Consistency"]


    if "prompt_strategy" not in st.session_state:
        st.session_state["prompt_strategy"] = strategies[0] # padrao é o step-by-step

    st.selectbox(
        "Escolha a Estratégia de Prompt",
        options=strategies,
        index=strategies.index(st.session_state["prompt_strategy"]),
        key="prompt_strategy" 
    )

    

    st.subheader(f"Estratégia selecionada: {st.session_state['prompt_strategy']}")
    ...
    def generateResposta(texto):
        ...
        blab = gera.BlabGPTAzul(strategy_name=st.session_state["prompt_strategy"])  
        #estrategia_transformada = strategy_mapping.get(estrategia_escolhida)

        chat['varial'][1] = texto
        chat['varial'][5] = st.session_state['tempo'] + str(chat['name']).replace(" ","_")         
        chat['varial'] = blab.escolheParte(chat['varial']) 
...
def main(): 
    ...
    st.sidebar.subheader(":blue[Interagindo:]")
    ...
    add_chat_button = st.sidebar.button(":blue[Criar Novo Chat]")
    ...
    st.sidebar.subheader(":blue[Chats:]")
    ...
    st.sidebar.divider()
    st.sidebar.subheader(":blue[Problemas:]")
    ...
    st.sidebar.write("Não esqueça de responder a pesquisa de avaliação da Blabinha no Google Forms https://forms.gle/hHZXs44TdUF4HENc7. A avaliação no Google Forms é APENAS sobre sua experiência no diálogo.")
